...
class SumTree(object):
    ...
    def add(self, p, data):
        ...
        if self.data_pointer >= self.capacity:  # replace when exceed the capacity
            self.data_pointer = 0
        if self.data_pointer % 99 == 1:
            print(self.data_pointer,self.tree[-self.capacity:],sum(self.tree[-self.capacity:]))
...
class Memory(object):  # stored as ( s, a, r, s_ ) in SumTree
    ...
    def store(self, transition):
        max_p = np.max(self.tree.tree[-self.tree.capacity:])  #max leaf value
        if max_p == 0:
            max_p = self.abs_err_upper
        self.tree.add(max_p, transition)   # set the max p for new p

    def sample(self, n):
        #samples indices， samples, importance sampling weights
    ...
    def batch_update(self, tree_idx, abs_errors):  #abs_error = |Q - Q'|
